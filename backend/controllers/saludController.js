...
// Eliminamos las importaciones de Dieta y Plato
// const Plato = require('../models/Plato');
// const Dieta = require('../models/Dieta');
// Importamos el nuevo modelo de PlanDieta
const PlanDieta = require('../models/PlanDieta');
// Necesitamos Usuario para los logros
const Usuario = require('../models/Usuario');
// Necesitamos el controlador de usuarios para chequear logros
const usuariosController = require('./usuariosController'); // Asegúrate que la ruta sea correcta
...
exports.actualizarPasos = async (req, res) => {
 try {
   ...
   // Chequea logros después de guardar
   const nuevosLogros = await usuariosController.chequearLogrosYDesbloquear(usuarioId);

   res.json({ mensaje: 'Datos de salud actualizados.', salud, nuevosLogros });
 } catch (error) {
...
// ---------------------------------------------------------------
// ---- FUNCIÓN ELIMINADA: actualizarKcalConsumidas -----------
// Ya no podemos sincronizar automáticamente desde la dieta
...
exports.obtenerHistorialSalud = async (req, res) => {
 try {
     ...
     if (!historial[i].kcalConsumidas || historial[i].kcalConsumidas === 0) {
       ...
       if (planDelMes && planDelMes.planGenerado && planDelMes.planGenerado.length > 0) {
         ...
         if (diaDietaEstimado && diaDietaEstimado.kcalDiaAprox > 0) {
           ...
           // await historial[i].save(); // Descomenta si quieres guardar la estimación
         }
       }
     }
   }
   ...
   for (let i = historial.length - 1; i >= 0; i--) {
     ...
     // Si fechaBD > fechaEsperada (ej. hoy), simplemente lo ignoramos y seguimos
   }
   // Comprueba si el registro de 'hoy' existe para la racha
   const registroHoy = historial.find(h => h.fecha.getTime() === hoy.getTime());
   if (registroHoy && racha === 0) { // Si solo hay registro de hoy, la racha es 1
      // Podríamos requerir un mínimo de pasos/kcal para contar el día, ej:
      // if (registroHoy.pasos > 100) racha = 1;
      // Por ahora, si existe el registro, cuenta.
      // Ajusta si la lógica anterior ya lo contó.
   }
...
exports.guardarDatosSalud = async (req, res) => {
 try {
   ...
   salud.pasos = pasos;
   salud.kcalQuemadas = kcalQuemadas; // Asume que este es el TOTAL quemado (pasos+manual)
   salud.kcalConsumidas = kcalConsumidas; // Kcal consumidas (probablemente manual)
   // Opcional: Si quieres guardar también el manual quemado, necesitarías recibirlo
   // salud.kcalQuemadasManual = req.body.kcalQuemadasManual || 0;

   await salud.save();
   const nuevosLogros = await usuariosController.chequearLogrosYDesbloquear(usuarioId);
   res.json({ mensaje: 'Datos de salud guardados.', salud, nuevosLogros });
 } catch (error) {
